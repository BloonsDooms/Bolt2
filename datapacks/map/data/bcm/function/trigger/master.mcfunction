...
execute if score @s master_menu matches 5 if score .is_busy item_structures matches 1 run function bcm:fail/tellraw {input:'"item structure system is busy loading or saving something else. please wait a few seconds, then try saving your map again. tell shuba if this error persists."'}
execute if score @s master_menu matches 5 unless score .is_busy item_structures matches 1 run function bcm:map/save
