...
$execute if score .success .calc matches 0 run tellraw @a [{color:"red",text:"Error occured trying to load "},{nbt:"maps[$(mapID)].mapName",storage:"maps:list",plain:true}," into editor"]